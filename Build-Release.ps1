...
if ($version -notmatch '^\d+\.\d+\.\d+$') { throw 'Ugyldig versjon i package.json.' }
...
if ((Get-FileHash -LiteralPath $runtimeZip -Algorithm SHA256).Hash.ToLowerInvariant() -ne $expectedHash) { throw 'Electron-kontrollsummen stemmer ikke.' }
...
Copy-Item -LiteralPath (Join-Path $appRoot 'release\Lag skrivebordssnarvei.vbs') -Destination $bundle
...
Write-Output "Release klar: $zipPath"
Write-Output "Utpakket app: $bundle"
